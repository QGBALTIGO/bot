...
# =========================
# DB safe wrappers (não quebra se faltar algo)
# =========================
...
def _buy_giro(db, user_id: int, price: int) -> bool:
    """
    Precisa existir algo como spend_coins_and_add_giro(user_id, price, giros=1)
    """
...
@app.get("/", response_class=HTMLResponse)
def root():
    return HTMLResponse("✅ Web rodando! Abra /shop para ver a Loja MiniApp.")
...
def api_shop_state(x_telegram_init_data: str = Header(default="")):
    ...
    return JSONResponse({"ok": True, "user_id": user_id, "coins": coins, "giros": giros, "fav_name": fav_name})


# ---------- API: listar para vender ----------
...
def api_sell_list(page: int = 1, q: str = "", x_telegram_init_data: str = Header(default="")):
    ...
    # normaliza + aplica busca (sem quebrar)
    ...
    return JSONResponse(
        {"ok": True, "page": page, "total_pages": total_pages, "total": total, "items": out, "sell_gain": SHOP_SELL_GAIN}
    )
...
def api_buy_giro(x_telegram_init_data: str = Header(default="")):
    ...
    return JSONResponse({"ok": True, "message": "🎡 <b>GIRO COMPRADO!</b>", "coins": coins, "giros": giros, "price": SHOP_GIRO_PRICE})
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .overlay{
      position:absolute;left:0;right:0;bottom:0;
      padding:10px;background:linear-gradient(180deg, rgba(0,0,0,0), rgba(0,0,0,.78));
    }
    ...
    .pill{
      position:absolute;top:10px;left:10px;
      padding:6px 10px;background:rgba(0,0,0,.45);
      border:1px solid rgba(255,255,255,.14);
    ...
    .heart{
      ...
      background: rgba(0,0,0,.45);
      border: 1px solid rgba(255,255,255,.14);
      font-size:16px;color: var(--heart);
    }
...
<body>
  ...
  <script>
      ...
      grid.querySelectorAll("button[data-sell]").forEach(btn=>{
        btn.onclick = async () => {
          ...
          try{
            ...
            if (!r.ok){
              setStatus("❌ Falha ao vender.", "err");
              return;
            }
            if (!r.data.ok){
              setStatus("⚠️ " + (r.data.error || "Não consegui vender."), "err");
              return;
            }
            ...
            await loadSell(); // recarrega página
          } finally {
            btn.disabled = false;
          }
        };
    ...
    async function loadState(){
      ...
      if (!r.ok){
        setStatus("❌ Falha ao conectar.", "err");
        return false;
      }
      coins = r.data.coins ?? 0;
      giros = r.data.giros ?? 0;
      favName = r.data.fav_name ?? "";
      updateStats();
      return true;
    }
    ...
    async function loadSell(){
      ...
      if (!r.ok){
        setStatus("❌ Falha ao carregar lista.", "err");
        return;
      }
      if (!r.data.ok){
        setStatus("⚠️ Erro ao carregar.", "err");
        return;
      }
      items = Array.isArray(r.data.items) ? r.data.items : [];
      totalPages = r.data.total_pages ?? 1;
      sellGain = r.data.sell_gain ?? 1;
    ...
    async function loadBuy(){
      // só atualiza texto e botão
      const buyText = document.getElementById("buyText");
      buyText.textContent = "Troque " + price + " coins por +1 giro.";
      updateStats();
    }
    ...
    document.getElementById("buyBtn").onclick = async () => {
      ...
      try{
        ...
        if (!r.ok){
          setStatus("❌ Falha ao comprar.", "err");
          return;
        }
        if (!r.data.ok){
          setStatus("⚠️ " + (r.data.error || "Você não tem coins suficientes."), "err");
          return;
        }
        coins = r.data.coins ?? coins;
        giros = r.data.giros ?? giros;
    ...
    (async () => {
      ...
      await loadSell();
      await loadBuy();
      // preço vem do backend via env (aqui fixo no texto)
      price = """ + str(SHOP_GIRO_PRICE) + r""";
      updateStats();
    })();
  </script>
</body>
