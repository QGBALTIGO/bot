...
from database import _run,add_dado_balance
...
@router.post("/shop/buy_dice")
def buy_dice(data:dict):
...
@router.post("/shop/buy_all_dice")
def buy_all_dice(data:dict):
...
def sell_character(data:dict):
    ...
    _run(
        "UPDATE users SET coins=coins+1 WHERE user_id=%s",
        (user_id,)
    )
...
@router.post("/shop/repurchase")
def repurchase(data:dict):
    ...
    if not sale or sale["repurchased"]:
        return {"ok":False}
...
<button class="buy" onclick="buyDice()">🎲 Comprar dado (2)</button>

<button class="buy" onclick="buyAllDice()">🎲 Comprar todos dados</button>
...
let r=await fetch(`/shop/data?user_id=${user_id}`)
let d=await r.json()
...
document.getElementById("repurchase").innerHTML=s
