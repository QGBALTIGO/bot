...
def parse_and_verify_state(state: str) -> int | None:
    # desfaz base64 urlsafe
    pad = "=" * (-len(state) % 4)
...
async def callback(
    ...
    if resp.status_code != 200:
        # log do erro no HTML (resumido)
        return HTMLResponse(f"<h3>Erro ao autorizar.</h3><pre>{resp.text[:800]}</pre>", status_code=400)
    ...
    # página final simples (sem precisar mexer no bot)
    return HTMLResponse(
