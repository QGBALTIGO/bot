# webapp.py — MiniApps (Coleção + Loja + Dado separado) + THEME PREMIUM (do theme.txt)
# ✅ Mantém 100% da lógica funcional (APIs/rotas/banco)
# ✅ Só troca/centraliza UI (CSS + layout + FX) usando o tema premium
# ✅ Continua: /app (coleção) | /shop (loja) | /dado (gacha)
...
# ============================================================
# THEME PREMIUM (centralizado)
# ============================================================
def _theme_css() -> str:
    # CSS premium (base) — mantém tudo consistente entre /app, /shop, /dado
    return r"""
:root{
  --bg0:#07070c;
  --bg1:#0c0b14;
...
.title h1{
  margin:0; font-size:18px; font-weight:1000; letter-spacing:.2px;
...
.toast.err{border-color: var(--bad); color: rgba(255,120,120,.95);}
...
.bdot{
  ...
  background: linear-gradient(180deg, rgba(255,43,74,.9), rgba(176,108,255,.8));
  box-shadow: 0 0 18px rgba(255,43,74,.18);
}
...
.hero{
  ...
  border:1px solid var(--stroke);
  background:
    radial-gradient(800px 320px at 10% 0%, rgba(255,43,74,.16), transparent 60%),
...
def _fx_js() -> str:
    # Canvas FX (partículas / glow) — leve e bonito
    return r"""
<script>
(() => {
  ...
  function draw(){
    ...
    const g = ctx.createRadialGradient(W*0.2, H*0.0, 10, W*0.2, H*0.0, Math.max(W,H));
    g.addColorStop(0,"rgba(255,43,74,0.10)");
    g.addColorStop(0.5,"rgba(176,108,255,0.07)");
...
def _tg_js_init() -> str:
    # Helpers JS compartilhados (initData + apiGet/apiPost + esc)
    return r"""
...
# =========================
# UI: /app — Coleção + Favoritos (SEM DADO) — PREMIUM
# =========================
...
.rarity .rdot{width:8px;height:8px;border-radius:99px;background:#fff;opacity:.9}
.tier-common .rarity .rdot{background:var(--c-common)}
.tier-rare   .rarity .rdot{background:var(--c-rare)}
.tier-epic   .rarity .rdot{background:var(--c-epic)}
.tier-mythic .rarity .rdot{background:var(--c-mythic)}
""" + "</style></head><body>"
        "<canvas id='fx'></canvas>"
        "<div class='wrap'>"
        "<div class='top'>"
        ...
        "<div class='heroRow'>"
        "<div class='heroLeft'>"
        "<div class='heroTitle'>🖼️ Galeria</div>"
        "<div class='heroHint'>Toque nos cards, use a busca e navegue por animes. Visual premium, sem mexer na lógica.</div>"
        "</div>"
...
<script>
  ...
  function renderCollection(){
    ...
    if (mode === "fav"){
      ...
      section.innerHTML = `
        <div class="sectionHeader"><div class="h">⭐ Favorito</div><div class="c">1</div></div>
        <div class="grid" id="favGrid"></div>
      `;
      root.appendChild(section);

      const grid = section.querySelector("#favGrid");
      const card = document.createElement("div");
      card.className = "card tier-mythic";
      card.innerHTML = `
        <div class="shine"></div>
        ${favImg ? `<img src="${esc(favImg)}" alt="">`
                : `<div style="height:220px;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.5)">Sem imagem</div>`}
        <div class="rarity"><span class="rdot"></span> FAVORITO</div>
        <div class="overlay">
    ...
    for (const g of groups){
      ...
      section.className = "section glass";
      section.innerHTML = `
        <div class="sectionHeader"><div class="h">${esc(g.title)}</div><div class="c">${g.cards.length}</div></div>
        <div class="grid"></div>
      `;
      ...
      for (const c of g.cards){
        ...
        card.innerHTML = `
          ...
          ${img ? `<img src="${esc(img)}" alt="">`
               : `<div style="height:220px;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.5)">Sem imagem</div>`}
          <div class="pillTag">x${qty} • ID ${charId}</div>
          <div class="rarity"><span class="rdot"></span> ${tier.replace("tier-","").toUpperCase()}</div>
          <div class="overlay">
...
# =========================
# UI: /dado — Dado separado (barra dado) — PREMIUM
# =========================
...
.optGrid{margin-top:14px;display:grid;grid-template-columns:repeat(2,1fr);gap:10px;}
.opt{
  text-align:left;
  border:1px solid rgba(255,255,255,.14);
  background:rgba(255,255,255,.04);
...
.statusLine{margin-top:12px;color:rgba(255,255,255,.75);font-size:12px;white-space:pre-wrap;}
...
.lname{font-weight:1000;font-size:16px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.lanime{margin-top:4px;color:rgba(255,255,255,.72);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
""" + "</style></head><body>"
        ...
        "<div class='top'>"
        "<div class='title'><h1>🎲 Dado</h1><div class='sub'>Role e escolha. Nº de opções = valor do dado.</div></div>"
        "<div class='pills'><div class='pill'>🎟️ <span id='bal'>-</span></div><div class='sep'></div>"
        "<div class='pill'>🎡 <span id='ext'>-</span></div></div>"
        "</div>"

        "<div class='hero'>"
        "<div class='heroRow'>"
        "<div class='heroLeft'>"
        "<div class='heroTitle'>🎁 Gacha</div>"
        "<div class='heroHint'>Se uma opção falhar, tente outra da mesma rolagem sem perder o dado. Se der erro do servidor, o dado é devolvido.</div>"
        "</div>"
        "<div class='badge'><span class='bdot'></span> BARRA DADO</div>"
        "</div></div>"
        ...
        "<div class='lname' id='nm'>...</div>"
        "<div class='lanime' id='an'>...</div>"
...
# =========================
# UI: /shop — Loja — PREMIUM (vender + comprar)
# =========================
...
.buyGrid{display:grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap:12px; margin-top:12px;}
@media (min-width: 520px){ .buyGrid{grid-template-columns: repeat(3, minmax(0,1fr));} }
...
.shopCard .bg{
  position:absolute; inset:-40%;
  background:
    radial-gradient(circle at 30% 30%, rgba(255,43,74,.18), transparent 45%),
    radial-gradient(circle at 70% 10%, rgba(176,108,255,.14), transparent 55%);
  opacity:.9; pointer-events:none;
}
.shopCard .shine{
  position:absolute; inset:-40%;
  background: radial-gradient(circle at 30% 30%, rgba(255,255,255,.16), transparent 38%);
  transform: rotate(12deg);
  opacity:.0; pointer-events:none;
  transition: opacity .2s ease;
  mix-blend-mode: screen;
}
.shopCard:hover .shine{opacity:.35}
.shopCard:active .shine{opacity:.45}
...
.smallBtn.primary{background: linear-gradient(90deg, rgba(255,43,74,.92), rgba(176,108,255,.70)); border:0; color:#fff;}
.smallBtn.ghost{border:1px solid rgba(255,255,255,.14); background:rgba(255,255,255,.05); color:#fff;}
...
""" + "</style></head><body>"
        ...
        "<div class='top'>"
        "<div class='title'><h1>🛒 Loja</h1><div class='sub' id='sub'>Carregando...</div></div>"
        "<div class='pills'><div class='pill'>🪙 <span id='coins'>-</span></div><div class='sep'></div>"
        "<div class='pill'>🎡 <span id='giros'>-</span></div></div>"
        "</div>"
        ...
        "<div class='heroRow'>"
        "<div class='heroLeft'>"
        "<div class='heroTitle'>🎡 Comprar GIRO</div>"
        "<div class='heroHint' id='buyText'>Carregando...</div>"
        "</div>"
        "<div class='badge'><span class='bdot'></span> LOJA</div>"
        "</div></div>"
        ...
        "<h3>🎡 1 GIRO</h3>"
        "<p>Troque coins por giros e continue rolando no gacha.</p>"
        "<div class='price' id='priceLine'>...</div>"
        "<button class='smallBtn primary' id='buyBtn' style='margin-top:12px;'>Comprar</button>"
        "</div>"
        "</div>"
        "</div>"
...
<script>
  ...
  function renderSell(){{
    ...
    if(!filtered.length){{
      root.innerHTML = "<div class='empty'>Nada para mostrar.</div>";
      return;
    }}
    ...
    for(const g of groups){{
      const section = document.createElement("div");
      section.className = "section glass";
      section.innerHTML = `<div class="sectionHeader"><div class="h">${{esc(g.title)}}</div><div class="c">${{g.items.length}}</div></div><div class="grid"></div>`;
      ...
      for(const c of g.items){{
        ...
        card.innerHTML = `
          ...
          ${{img ? `<img src="${{esc(img)}}" alt="">`
                : `<div style="height:220px;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.5)">Sem imagem</div>`}}
          <div class="pillTag">x${{qty}} • ID ${{c.character_id}}</div>
          <div class="overlay">
            <div class="name">${{esc(c.character_name || "Personagem")}}</div>
            <div class="meta">${{esc(c.anime_title || "")}}</div>
            <div class="actions">
              <button class="smallBtn primary" data-sell="${{c.character_id}}">Vender (+${{sellGain}})</button>
            </div>
          </div>
        `;
        grid.appendChild(card);
    ...
    root.querySelectorAll("button[data-sell]").forEach(btn => {{
      btn.onclick = async () => {{
        ...
        btn.disabled = true;
        try{{
          setToast("Processando venda...", "");
          const r = await apiPost("/api/shop/sell/confirm", {{ character_id: id }});
          if (!r.ok) {{ setToast("❌ Falha ao vender.", "err"); return; }}
          if (!r.data.ok) {{ setToast("⚠️ " + (r.data.error || "Não consegui vender."), "err"); return; }}

          coins = r.data.coins ?? coins;
          giros = r.data.giros ?? giros;
          updateStats("sell");
          setToast("✅ Venda concluída!", "ok");
          await loadSell();
        }} finally {{
          btn.disabled = false;
        }}
  ...
  async function loadState(){{
    ...
    if(!r.ok){{ setToast("❌ Falha ao conectar.", "err"); return false; }}
  ...
  async function loadSell(){{
    ...
    if(!r.ok){{ setToast("❌ Falha ao carregar lista.", "err"); return; }}
    if(!r.data.ok){{ setToast("⚠️ " + (r.data.error || "Erro ao carregar."), "err"); return; }}
  ...
  async function loadBuy(){{
    ...
    document.getElementById("priceLine").textContent = "Preço: " + price + " coins";
    updateStats("buy");
  }}
  ...
  document.getElementById("buyBtn").onclick = async () => {{
    ...
    btn.disabled = true;
    try{{
      setToast("Processando compra...", "");
      const r = await apiPost("/api/shop/buy/giro", {{}});
      if(!r.ok){{ setToast("❌ Falha ao comprar.", "err"); return; }}
      if(!r.data.ok){{ setToast("⚠️ " + (r.data.error || "Você não tem coins suficientes."), "err"); return; }}

      coins = r.data.coins ?? coins;
      giros = r.data.giros ?? giros;
      updateStats("buy");
      setToast("✅ GIRO comprado!", "ok");
    }} finally {{
      btn.disabled = false;
    }}
  }};
