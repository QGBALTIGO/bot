...
from identity_repository import get_identity, public_display_name
...
def _safe_message_summary(user_id: int) -> dict[str, int]:
    try:
        ...
        return {"received": len(state.get("inbox") or []), "sent": len(state.get("sent") or [])}
    except Exception:
...
def universal_search(user_id: int, query: str, limit: int = 24) -> dict[str, Any]:
    ...
    with pool.connection() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """SELECT user_id,nickname,telegram_full_name,telegram_username FROM user_identity_v2
                   WHERE private_profile=FALSE AND (nickname ILIKE %s OR telegram_full_name ILIKE %s OR telegram_username ILIKE %s)
                   ORDER BY CASE WHEN LOWER(COALESCE(nickname,''))=LOWER(%s) THEN 0 ELSE 1 END,updated_at DESC LIMIT %s""",
                (f"%{q}%", f"%{q}%", f"%{q}%", q, max(1, min(20, limit))),
            )
            for row in cur.fetchall() or []:
                display = str(row.get("nickname") or row.get("telegram_full_name") or "").strip()
                if not display and row.get("telegram_username"):
                    display = f"@{row['telegram_username']}"
                people.append({"user_id": int(row.get("user_id") or 0), "display_name": display or "Jogador"})
