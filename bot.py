...
async def _ensure_top_cache_fresh():
    async with _REFRESH_LOCK:
        ...
        if items:
            # ✅ FIX: replace_top_anime_cache precisa de updated_at
            replace_top_anime_cache(items, updated_at=now)
...
# Cache simples por anime (reduz falhas e chamadas)
_CHAR_POOL_CACHE: dict[int, dict] = {}  # anime_id -> {exp:int, title:str, cover:str, chars:[{id,name,image}]}
...
async def _anilist_random_character_from_anime(anime_id: int, tries: int = 12) -> Optional[dict]:
    """
    Retorna SEMPRE que possível um personagem.
    Se o personagem não tiver foto, usa cover do anime como fallback (quase zera falha).
    """
    now = int(time.time())
    ...
    for c in pick_list:
        ...
        if img:
            return {
                ...
                "name": c["name"],
                "image": img,
                "anime_title": cached.get("title") or "Obra"
            }

    cover = cached.get("cover") or DADO_FALLBACK_IMAGE
    c = pick_list[0]
    return {
        "id": int(c["id"]),
        "name": c["name"],
...
async def dado_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ FIX: se DB falhar ao criar o roll, reembolsa e não “some o resto”
    try:
        # compatível com create_dice_roll(old) e create_dice_roll(new)
        ...
        # fallback para assinatura antiga (sem status/created_at)
        try:
    ...
    # ✅ FIX: se a imagem falhar, manda texto + botões
    try:
        await update.message.reply_photo(
...
async def callback_dado_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with roll_lock:
        ...
        ok_lock_db = try_set_dice_roll_status(roll_id, expected="pending", new_status="processing")
        if not ok_lock_db:
            await q.answer("Esse dado já foi usado.", show_alert=True)
            return
    ...
    try:
        info = await _anilist_random_character_from_anime(anime_id, tries=12)
    except Exception:
        info = None
    ...
    if not info:
        ...
        await q.message.reply_html(
            "⚠️ <b>Esse anime está com poucas informações agora.</b>\n\n"
...
def _get_colecao_lock(owner_id: int) -> asyncio.Lock:
    lock = _colecao_locks.get(int(owner_id))
    if lock is None:
        lock = asyncio.Lock()
        _colecao_locks[int(owner_id)] = lock
    return lock
...
def _pick_anime(row: dict) -> str:
    for k in ("anime_title", "anime", "anime_name", "obra", "title", "series"):
        v = _safe_str(row.get(k))
        if v:
            return v
    return ""
...
def _pick_char_id(row: dict) -> int:
    for k in ("character_id", "char_id", "id", "card_id"):
        v = row.get(k)
        if v is not None:
            n = _safe_int(v, 0)
            if n:
...
def _pick_qty(row: dict) -> int:
    for k in ("quantity", "qty", "qtd", "count", "amount"):
        v = row.get(k)
        if v is not None:
            n = _safe_int(v, 1)
            return max(1, n)
...
def _pick_image(row: dict) -> Tuple[str, bool]:
    """
    Retorna (url, is_custom).
    Se tiver custom_image (setfoto), prioriza ela.
    """
    custom = _safe_str(row.get("custom_image"))
    if custom:
        return custom, True
    img = _safe_str(row.get("image"))
    return img, False
...
def _paginate_cards(all_cards: List[dict], page: int) -> Tuple[List[dict], int, int]:
    ...
    end = min(total, start + ITENS_POR_PAGINA)
...
async def enviar_colecao_by_owner(
    owner_id: int,
    first_name: str,
    update: Update,
    ...
    *,
    ...
    row = db.get_user_row(owner_id) or {}
    fav_image = _safe_str(row.get("fav_image")) or ""
    fav_name = _safe_str(row.get("fav_name")) or ""
...
async def colecao_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with lock:
        await enviar_colecao_by_owner(owner_id, update.effective_user.first_name, update, context, 1, edit=False)
...
def main():
    ...
    app.add_handler(CommandHandler("eventostart", eventostart))
    app.add_handler(CommandHandler("eventostop", eventostop))
    app.add_handler(CallbackQueryHandler(callback_admin, pattern=r"^admin:"))
...
# ==================================================
# BALTIGO ENGINE — IMPLEMENTAÇÃO (NOVOS COMANDOS)
# ==================================================

MINIAPP_BASE = "https://bot-production-1980.up.railway.app"

async def summon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/summon — reaproveita o /dado (não remove nada, só oferece atalho premium)."""
    # Se /dado for callback-only em algum fluxo, isso ainda funciona porque chama o handler atual.
    return await dado_command(update, context)

async def market(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/market [pagina] — lista ofertas do mercado global."""
    from database import market_list as db_market_list
    ...
    items = db_market_list(page=page, per_page=10) or []
    if not items:
        await update.message.reply_html(
            "🛒 <b>MERCADO GLOBAL</b>\n\n"
            "<i>Nenhuma oferta no momento.</i>\n\n"
            "Para vender: <code>/vender ID PREÇO</code>"
        )
        return

    lines = [f"🛒 <b>MERCADO GLOBAL</b>\n\n<i>Página {page}</i>\n"]
    for it in items:
        lines.append(
            f"<b>#{it['listing_id']}</b> — <code>{it['character_id']}</code>. "
            f"{it['character_name']} — <b>{it['price']}</b> coins"
        )

    lines.append("\n✅ Comprar: <code>/comprar LISTING_ID</code>")
    lines.append("💰 Vender: <code>/vender ID PREÇO</code>")

    await update.message.reply_html("\n".join(lines))

async def vender(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/vender <char_id> <preco> — cria listing e remove 1 unidade da sua coleção."""
    if not getattr(context, "args", None) or len(context.args) < 2:
        await update.message.reply_html("Use: <code>/vender ID PREÇO</code>")
        return
    ...
    from database import market_create_listing
    ok, msg = market_create_listing(update.effective_user.id, char_id, price)
    if ok:
        await update.message.reply_html(f"✅ <b>Venda criada!</b>\n\n{msg}\n\nUse <code>/market</code> para ver.")
    else:
        await update.message.reply_html(f"❌ Não consegui vender: <b>{msg}</b>")

async def comprar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/comprar <listing_id> — compra e entrega na coleção."""
    if not getattr(context, "args", None) or len(context.args) < 1:
        await update.message.reply_html("Use: <code>/comprar LISTING_ID</code>")
        return
    ...
    from database import market_buy
    ok, msg = market_buy(update.effective_user.id, listing_id)
    if ok:
        await update.message.reply_html("✅ <b>Compra concluída!</b>\n\nO personagem já entrou na sua coleção.")
    else:
        await update.message.reply_html(f"❌ Não consegui comprar: <b>{msg}</b>")

async def evento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/evento — mostra o evento ativo no momento."""
    from database import get_active_event
    ev = get_active_event()
    if not ev:
        await update.message.reply_html("🎉 <b>EVENTOS</b>\n\n<i>Nenhum evento ativo agora.</i>")
        return

    start = int(ev.get("start_time") or 0)
    end = int(ev.get("end_time") or 0)

    await update.message.reply_html(
        "🎉 <b>EVENTO ATIVO</b>\n\n"
        f"🔥 <b>{ev.get('event_name')}</b>\n"
        f"⏳ Início: <code>{start}</code>\n"
        f"⌛ Fim: <code>{end}</code>"
    )

async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/admin — dashboard com botões."""
    uid = update.effective_user.id
    if uid not in ADMINS:
        await update.message.reply_html("❌ Apenas admins.")
        return

    from database import get_global_stats
    st = get_global_stats() or {}

    kb = [
        [InlineKeyboardButton("📊 Atualizar", callback_data="admin:refresh")],
        [InlineKeyboardButton("🌐 Abrir Coleção (MiniApp)", url=f"{MINIAPP_BASE}/app")],
        [InlineKeyboardButton("🛒 Abrir Loja (MiniApp)", url=f"{MINIAPP_BASE}/shop")],
        [InlineKeyboardButton("🛒 Ver Mercado", callback_data="admin:market")],
    ]

    await update.message.reply_html(
        "📊 <b>DASHBOARD ADMIN</b>\n\n"
        f"👥 Usuários: <b>{st.get('users', 0)}</b>\n"
        f"🪙 Coins totais: <b>{st.get('coins', 0)}</b>\n"
        f"🎴 Personagens: <b>{st.get('chars', 0)}</b>\n"
        f"🛒 Listings: <b>{st.get('market', 0)}</b>",
...
async def callback_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = q.from_user.id
    if uid not in ADMINS:
        await q.answer("Apenas admins.", show_alert=True)
        return
    await q.answer()

    data = q.data or ""
    from database import get_global_stats, market_list as db_market_list

    if data == "admin:market":
        items = db_market_list(page=1, per_page=10) or []
        if not items:
            await q.edit_message_text("🛒 MERCADO GLOBAL\n\nNenhuma oferta no momento.")
            return
        text = "🛒 MERCADO GLOBAL\n\n"
        for it in items:
            text += f"#{it['listing_id']} — {it['character_name']} — {it['price']} coins\n"
        text += "\nComprar: /comprar LISTING_ID"
    ...
    await q.edit_message_text(
        "📊 DASHBOARD ADMIN\n\n"
        f"👥 Usuários: {st.get('users', 0)}\n"
        f"🪙 Coins totais: {st.get('coins', 0)}\n"
        f"🎴 Personagens: {st.get('chars', 0)}\n"
        f"🛒 Listings: {st.get('market', 0)}"
    )

async def eventostart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/eventostart NOME HORAS — inicia evento (admin)."""
    uid = update.effective_user.id
    if uid not in ADMINS:
        await update.message.reply_html("❌ Apenas admins.")
        return
    ...
    if not getattr(context, "args", None) or len(context.args) < 2:
        await update.message.reply_html("Use: <code>/eventostart NOME HORAS</code>\nEx: <code>/eventostart NarutoWeek 72</code>")
        return
    ...
    try:
        ...
        await update.message.reply_html("HORAS inválido.")
        return

    from database import start_event
    start_event(name, hours, created_by=uid)
    await update.message.reply_html(f"✅ Evento iniciado: <b>{name}</b> por <b>{hours}h</b>")

async def eventostop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/eventostop — encerra eventos (admin)."""
    uid = update.effective_user.id
    if uid not in ADMINS:
        await update.message.reply_html("❌ Apenas admins.")
        return
    from database import stop_all_events
    stop_all_events()
    await update.message.reply_html("✅ Todos eventos foram encerrados.")
