...
def _session_key() -> bytes:
    explicit = str(os.getenv("ANINEXUS_SESSION_SECRET", "") or "").strip()
    material = explicit or f"aninexus-source-session:{_bot_token()}"
...
def _not_connected(message: str = "Este recurso ainda está em integração com o Source.") -> JSONResponse:
    return JSONResponse(
        {
            "error": {
...
def build_aninexus_compat_router() -> APIRouter:
    ...
    def bot_info(authorization: str = Header(default="")):
        try:
            ...
            return _unauthorized(str(exc))
        # Mantém o fallback visual original do frontend (ANINEXUS).
        return JSONResponse({"name": "AniNexus"})
            ...
            return JSONResponse(
                {
                    "error": {
                        ...
                        "message": "A consulta de coleção de outro usuário ainda não está disponível.",
                    }
                },
                status_code=403,
            )
