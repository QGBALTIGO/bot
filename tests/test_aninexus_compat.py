...
from webapp_routes import aninexus_compat


def test_seal_session_roundtrip(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BOT_TOKEN", "123456789:test-secret")
    monkeypatch.delenv("ANINEXUS_SESSION_SECRET", raising=False)

    token = aninexus_compat._issue_session(
        {
            "id": 42,
            "first_name": "Luffy",
            "last_name": "Monkey",
    ...
    payload = aninexus_compat._read_session(token)
...
def test_seal_session_rejects_tampering(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BOT_TOKEN", "123456789:test-secret")
    token = aninexus_compat._issue_session({"id": 42, "first_name": "Luffy"})
    body, signature = token.rsplit(".", 1)
    tampered = ("A" if body[0] != "A" else "B") + body[1:] + "." + signature

    with pytest.raises(ValueError):
        aninexus_compat._read_session(tampered)


def test_character_payload_preserves_source_identity() -> None:
    payload = aninexus_compat._character_payload(
        {
            "id": 184,
            "name": "Monkey D. Luffy",
            "anime": "One Piece",
